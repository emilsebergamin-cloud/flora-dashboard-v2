...
import { Plus, Trash2, ArrowUpRight, Zap, Equal, Minus, Pencil } from 'lucide-react';
...
export default function TemasFuturos() {
  ...
  const items = (dashboard.ideas?.temasFuturos ?? []).slice().sort((a, b) => {
    ...
    const byPrio = (order[a.prioridad] ?? 9) - (order[b.prioridad] ?? 9);
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((t) => {
            ...
            return (
              <div key={t.id} className="bg-blanco border border-beige-2 rounded-2xl px-5 py-4 flex items-start gap-3">
