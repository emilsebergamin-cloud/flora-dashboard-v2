...
export default function Reflexiones() {
  ...
  const [editDraft, setEditDraft] = useState({ text: '', tag: '', fecha: '' });
  ...
  const startEdit = (r) => {
    setEditingId(r.id);
    setEditDraft({ text: r.text, tag: r.tag ?? '', fecha: r.fecha ?? '' });
  };

  const handleUpdate = (e) => {
    e.preventDefault();
    if (!editDraft.text.trim()) return;
    update((d) => updateReflexion(d, editingId, editDraft));
    setEditingId(null);
  };
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {items.map((r) => {
            if (editingId === r.id) {
              return (
                <form key={r.id} onSubmit={handleUpdate}
                  className="bg-blanco border border-rosa-claro rounded-2xl px-5 py-4 flex flex-col gap-3">
                  <textarea className="textarea-flora" rows={5} value={editDraft.text} autoFocus
                    onChange={(e) => setEditDraft({ ...editDraft, text: e.target.value })} />
                  <div className="grid grid-cols-2 gap-3">
                    <input type="date" className="input-flora" value={editDraft.fecha}
                      onChange={(e) => setEditDraft({ ...editDraft, fecha: e.target.value })} />
              ...
              <article key={r.id} className="bg-blanco border border-beige-2 rounded-2xl px-5 py-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => startEdit(r)} className="text-texto-suave hover:text-texto p-1 transition-colors">
